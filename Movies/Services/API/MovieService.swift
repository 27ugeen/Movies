...
protocol MovieServiceProtocol {
    ...
    func fetchMovieVideos(id: Int, completion: @escaping (Result<[MovieVideo], Error>) -> Void)
}
...
//            URLQueryItem(name: "include_video", value: "true")
    ...
    func searchMovies(query: String, page: Int, completion: @escaping (Result<[Movie], Error>) -> Void) {
        ...
        apiClient.performRequest(endpoint: "search/movie", queryItems: queryItems) { (result: Result<MovieResponse, Error>) in
            switch result {
            case .success(let response):
//                print("Found movies: \(response.results)")
                completion(.success(response.results))
            case .failure(let error):
                completion(.failure(error))
            }
    ...
    func fetchMovieVideos(id: Int, completion: @escaping (Result<[MovieVideo], Error>) -> Void) {
        ...
        apiClient.performRequest(endpoint: endpoint, queryItems: queryItems) { (result: Result<MovieVideosResponse, Error>) in
            switch result {
            case .success(let response):
                completion(.success(response.results))
            case .failure(let error):
